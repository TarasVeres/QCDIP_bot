...
async def start(message: types.Message):
    ...
    if message.chat.id == message.from_user.id:
        if str(message.chat.id) in Sheet['Access_id']:
            ...
            Reply_message[m_id]['language'] = Sheet['Access_id_language'][str(m_id)][-1]
            Reply_message[m_id] = Deleter.language(Reply_message[m_id], Sheet)
            txt = Sheet['languages']['59'][Reply_message[m_id]['language']]
...
async def start(message: types.Message):
    ...
    if message.chat.id == message.from_user.id:
        ...
        if str(message.chat.id) in Sheet['Access_id']:
            ...
            Reply_message[m_id] = dict()
            Reply_message[m_id]['language'] = Sheet['Access_id_language'][str(m_id)][-1]
            Reply_message[m_id] = Deleter.language(Reply_message[m_id], Sheet, m_id)
            Reply_message[message.chat.id]['record'] = Sheet['Access_id'][str(message.chat.id)]
            button = types.InlineKeyboardMarkup(row_width=2)
            button.add(types.InlineKeyboardButton(text='🇺🇦Українська', callback_data='UA'),
                       types.InlineKeyboardButton(text='🇱🇷English', callback_data='EN'))
            button.add(types.InlineKeyboardButton(text='🇹🇷Türk', callback_data='TR'))
            await bot.send_message(message.chat.id, languages, reply_markup=button)
        else:
            await bot.send_message(message.chat.id, languages_non)
    else:
        pass
    ...
    if message.chat.id == message.from_user.id:
        if str(message.chat.id) in Sheet['Access_id']:
            if mq == '':
                ...
                Reply_message[m_id]['language'] = Sheet['Access_id_language'][str(m_id)][3]
                Reply_message[m_id] = Deleter.language(Reply_message[m_id], Sheet)
...
async def device_callback(call: types.CallbackQuery):
    global mq, Reply_message, backer, Sheet
    button = types.InlineKeyboardMarkup(row_width=2)
    try:
        mq = ''
        c_id = call.from_user.id
        Reply_message[c_id]['stoper'] = 'good'
        if call.data in ('UA', 'EN', 'TR'):
            writer.writer_languages(str(call.from_user.id), Sheet, call.data)
            Sheet = update_sheet()
            Reply_message[c_id]['language'] = Sheet['Access_id_language'][str(c_id)][3]
            Reply_message[c_id] = Deleter.language(Reply_message[c_id], Sheet, call.message.chat.id)
            tel = writer.print_text(Reply_message[c_id], 0, Sheet)
            await bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text=tel)
        elif (call.data in Sheet['Location']) and not Sheet['Location'][call.data]:
